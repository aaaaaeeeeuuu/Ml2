...
-- ============================================================================
-- CRYSTALS TAB (UPDATED WITH INSTANT-GET SECRET CHAOS SORCERER)
-- ============================================================================
local crystalTab = window:AddTab("Crystals")

local autoOpenActive = false
local instantChaosActive = false
local selectedCrystal = "Secret Blades Crystal"
...
-- ⚡ TURBO INSTANT 100% GET: SECRET CHAOS SORCERER
crystalTab:AddSwitch("⚡ Instant-Get Secret Chaos Sorcerer", function(state)
    instantChaosActive = state
    
    if instantChaosActive then
        task.spawn(function()
            local rEvents = game:GetService("ReplicatedStorage"):WaitForChild("rEvents", 5)
            local openRemote = rEvents and rEvents:FindFirstChild("openCrystalRemote")
            local sellRemote = rEvents and rEvents:FindFirstChild("sellPetEvent")
            local petsFolder = player:FindFirstChild("petsFolder") or player:FindFirstChild("pets")
            
            while instantChaosActive do
                -- 1. Check if the player already unlocked Secret Chaos Sorcerer
                local hasSorcerer = false
                if petsFolder then
                    for _, pet in pairs(petsFolder:GetChildren()) do
                        if string.find(pet.Name, "Secret Chaos Sorcerer") or string.find(pet.Name, "Chaos Sorcerer") then
                            hasSorcerer = true
                            break
                        end
                    end
                ...
                if hasSorcerer then
                    pcall(function()
                        game:GetService("StarterGui"):SetCore("SendNotification", {
                            Title = "PET UNLOCKED!",
                            Text = "Secret Chaos Sorcerer obtained successfully!",
                            Duration = 8
                        })
                    end)
                    instantChaosActive = false
                    break
                end

                -- 2. Multi-Threaded Packet Blast (Cracks eggs concurrently with 0 delay)
                for i = 1, 8 do
                    task.spawn(function()
                        if openRemote then
                            pcall(function()
                                openRemote:InvokeServer("openCrystal", "Secret Blades Crystal")
                            end)
                        end
                    end)
                end

                -- 3. Auto-trash non-target pets so the inventory never gets full
                if sellRemote and petsFolder then
                    for _, pet in pairs(petsFolder:GetChildren()) do
                        if not string.find(pet.Name, "Chaos Sorcerer") and not string.find(pet.Name, "Secret Chaos Sorcerer") then
                            pcall(function()
                ...
                task.wait(0.03) -- Safe burst cycle frequency
            end
        end)
    end
end)
...
crystalTab:AddSwitch("Auto Sell Corrupted Elements Hydra", function(state)
...
crystalTab:AddSwitch("Auto Sell Dual Destiny Shadow Dragon", function(state)
