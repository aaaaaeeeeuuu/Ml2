...
task.spawn(function()
    ...
    -- 🎨 DROPDOWN BOX COLOR (Baguhin mo dito kung gusto mo ibang kulay)
    ...
    local function forceFireTheme(element)
        ...
        -- Detect kung ito yung mismong Box ng dropdown batay sa ibinigay mo
        local isDropdownBox = (nameLower == "box") and (parentName:match("dropdown") or parentName == "d" or parentName:match("adddropdown"))

        -- 1. IMAGE ASSETS (Dito papasok yung Box mo dahil ImageLabel/ImageButton ito)
...
-- 2. Build the Timer Frame Layout (Fixed Position & Fully Transparent)
...
-- Visually toggle Elerium switch checkmark to ON immediately upon loading script
...
-- Checks if the UI element is any type of standard visual Frame container
...
local function hideElement(element)
    ...
    -- Save original properties before modifying them
...
-- FAST PUNCH FEATURE
-- ============================================================================
local FastPunchConnection -- Local variable to store the Heartbeat connection cleanly
...
mainTab:AddSwitch('Fast Punch', function(state)
    ...
    if state then
        ...
        -- Clean up any existing connection to prevent thread stacking
            ...
            if _G.punchanim then
                ...
                if Character then
                    ...
                    if PunchTool then
                        ...
                        -- Auto equip if tool is in backpack
        ...
        -- Turn off loop safely
        if FastPunchConnection then 
            FastPunchConnection:Disconnect() 
            FastPunchConnection = nil 
        end
        
        -- Reset attack timing to default game mechanics
...
-- 1. Create Dropdown to select the target crystal
local crystalDropdown = mainTab:AddDropdown("Select Crystal", function(choice)
...
mainTab:AddSwitch("Auto Open Selected", function(state)
    ...
    -- Fire loop execution asynchronously to prevent locking the user interface thread
    task.spawn(function()
        while autoOpenActive do
            local args = {
                [1] = "openCrystal",
                [2] = selectedCrystal, -- Dynamically passes the dropdown choice string
                [3] = 1
            }
            
            -- Network invocation block
            game:GetService("ReplicatedStorage").rEvents.openCrystalRemote:InvokeServer(table.unpack(args))
            
            -- Essential structural cycle rest interval
            task.wait(0.1)
        end
    end)
end)
...
mainTab:AddLabel("Auto Evolved")

-- Variable tracking the active execution state of our loop
local autoEvolveEnabled = false

-- Add a functional switch into the Automation tab
mainTab:AddSwitch("Auto Evolve All Pets", function(state)
    autoEvolveEnabled = state
    
    if autoEvolveEnabled then
        print(" Auto-Evolve wrapper enabled.")
        ...
        task.spawn(function()
            ...
            if not autoEvolve or not autoEvolve:IsA("RemoteFunction") then
                warn("autoEvolveRemote not found.")
                return
            end
            ...
            while autoEvolveEnabled do
                local success, result = pcall(function()
                    return autoEvolve:InvokeServer()
                end)
                
                if not success then
                    warn(" Invocation error: " .. tostring(result))
                end
                
                task.wait(0.1) -- Prevents rapid rate-limiting or crash flags
            end
            print("Auto-Evolve thread terminated.")
        end)
    else
        print("Auto-Evolve wrapper disabled.")
    end
end)
...
-- Add Options to Dropdown Layout
...
_Teleport:AddButton('Teleport Now', function()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        if selectedLocationCFrame then
            -- Wipe local physics momentum to block server rubberbanding
            character.HumanoidRootPart.Velocity = Vector3.new(0, 0, 0)
            character.HumanoidRootPart.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
            
            -- Teleport client assembly safely
            character:PivotTo(selectedLocationCFrame)
        else
            warn('No location selected inside dropdown menu.')
        end
    else
        warn('Character root structure not fully loaded.')
    end
end)
...
-- Create tab after vars are ready
local Tab = window:AddTab("☠️ Auto Kill") -- Name of tab
...
-- [[ UPDATED: Target Kill List Features ]]

-- Kill List UI Label Display (Elerium V2)
...
local targetKillDropdown = Tab:AddDropdown("Add To Target Kill List", function(text)
    ...
    if target then
        ...
        print("[TARGET KILL LIST] Added: " .. target.Name .. " (" .. target.DisplayName .. ")")
        UpdateKillListLabel()
    end
end)

-- Keep target kill dropdown updated with players (Display Name only)
task.spawn(function()
    while true do
        task.wait(3)
        pcall(function()
...
Tab:AddButton("Clear All Target Kill List", function()
    TargetKillPlayers = {}
    print("[TARGET KILL LIST] Target Kill List cleared completely.")
    UpdateKillListLabel()
end)
...
-- [[ UPDATED: Whitelist List Features ]]

-- Whitelist UI Label Display (Elerium V2)
...
Tab:AddButton("Add Whitelist Players", function()
    if MultiInputText == "" then 
        print("[WHITELIST] Text box is empty!")
        return 
    end
    ...
    for nameSegment in string.gmatch(MultiInputText, "([^,]+)") do
        ...
        if target then
            ...
            print("[WHITELIST] Added: " + target.Name + " (" + target.DisplayName + ")")
        else
            local cleanName = nameSegment:match("^%s*(.-)%s*$")
            print("[WHITELIST] Could not find player: '" + cleanName + "'")
        end
    end
...
Tab:AddButton("Clear All Whitelist", function()
    WhitelistedPlayers = {}
    print("[WHITELIST] Whitelist cleared completely.")
    UpdateWhitelistLabel()
end)

local Tab = window:AddTab("⚙️ Misc") -- Name of tab
